...
/**
 * SPTerms are Java representations of Prolog terms. SPTerms do
 * <i>not</i> contain Prolog terms themselves; Prolog terms are
 * <i>always</i> stored on the Prolog heap. Instead, an SPTerm object
 * contains an <i>SP_term_ref</i> which is an index into a stack maintained by
 * Prolog. This stack in turn contains references into the Prolog
 * heap. For more information, consult the section on
 * "Mixing Java and Prolog" in the SICStus Prolog User's Manual.
 * 
 * <p> In other words, SPTerm objects are only handles to
 * SP_term_refs. The actual life of an SP_term_ref (and of the Prolog term)
 * are completely independent of the life of an SPTerm object. So, if a
 * SPTerm object is garbage collected, the SP_term_ref and the Prolog
 * term may very well be alive and well inside the Prolog stack. On
 * the other hand, just because an SPTerm object is not GC:d, that does
 * not mean that the Prolog term is still alive. The life-span of
 * SP_term_refs and Prolog terms are explained in detail in the User's
 * Manual Section "SPTerm and Memory".
 *
 * <p> The methods in this class are encapsulations of the C-functions in
 * the C-Prolog interface which accesses SP_term_refs and their
 * underlying Prolog terms.
 *
 * <p> You should not inherit from <code>SPTerm</code>.
 */

public class SPTerm implements Term
{
    // Object dummy [] = new Object[100];;               // [PM] DUMMY REMOVE THIS BEFORE RELEASE
    ...
    // int termAgeTick;            // Age of term. Should never be younger (larger integer) than this.age()
    ...
    static final long SP_ILLEGAL_TERM_REF = 0; // [PM] 3.8.5 signifies Prolog deallocated this SP_term_ref

    // [PM] 3.9 moved to SICSTus
    //  private native String spGetListChars( SICStus sp, long term ) throws NativeCodeException;
    //  private native String spGetNumberChars( SICStus sp, long term ) throws NativeCodeException;
    //  private native String spGetString( SICStus sp, long term ) throws NativeCodeException;
    ...
    //  private native double spGetFloat( SICStus sp, long termref ) throws NativeCodeException;
    //  private native Object spGetObject( SICStus sp, long termref ) throws NativeCodeException;
    //  private native int spCompare( SICStus sp, long term1, long term2 );
    //  private native void spConsFunctor( SICStus sp, long term, long atm, long args[]) throws NativeCodeException;
    //  private native void spConsList( SICStus sp, long termref, long head, long tail ) throws NativeCodeException;  
    //  private native int spGetFunctorArity( SICStus sp, long term ) throws NativeCodeException;
    //  private native void spPutFloat( SICStus sp, long termref, double d ) throws NativeCodeException;
    //  private native void spPutFunctor( SICStus sp, long termref, long atm, int arity) throws NativeCodeException;
    //  private native void spPutInteger( SICStus sp, long termref, long i ) throws NativeCodeException;
    //  private native void spPutList( SICStus sp, long termref ) throws NativeCodeException;  
    //  private native void spPutListChars( SICStus sp, long termref, String string ) throws NativeCodeException;  
    //  private native void spPutNumberChars( SICStus sp, long termref, String string ) throws NativeCodeException;  
    //  private native void spPutString( SICStus sp, long termref, String value ) throws NativeCodeException;
    //  private native void spPutVariable( SICStus sp, long termref ) throws NativeCodeException;  
    //  private native int spTermType( SICStus sp, long term);
    //  private native long spCreateGlobalRef( SICStus sp, Object obj );
    //  private native void spGetArg( SICStus sp, long i, long term, long arg ) throws NativeCodeException;
    //  private native long spGetAtom( SICStus sp, long term ) throws NativeCodeException;
    //  private native long spGetAtomFromString( SICStus sp, String string );
    //  private native long spGetFunctorCanonical( SICStus sp, long term ) throws NativeCodeException;
    //  private native long spGetInteger( SICStus sp, long termref ) throws NativeCodeException;
    //
    ...
    //
    //  private native void spGetList( SICStus sp, long term, long head, long tail ) throws NativeCodeException;
    //  private native void spPutAtom( SICStus sp, long termref, long canonical ) throws NativeCodeException;
    //  private native void spPutTerm( SICStus sp, long term1, long term2 ) throws NativeCodeException;
    ...
    // [PM] 3.9.1b4 The SPTerm constructors in 3.9.0 was not declared to throw IllegalCallerException (or even SPException).
    //      To maintain source compatibility we fake out and throw a NullPointerException instead.
...
// [PD] 3.9.2
//            throw new NullPointerException("Creating SPTerm in wrong thread. (IllegalCallerException in disguise.)");
            throw new NullPointerException("Creating or accessing SPTerm in wrong thread. (IllegalCallerException in disguise.)");
        }
    }
    ...
    {
        if (debugging(2)) {
            SICStus.dbgPrintln("Enter: " + superToString() + ".initSPTerm(" + sp + ", " + nativeTermRef + ", " + fromGlue + ")");
        }
        ...
        synchronized (sp) {
            ...
            if (debugging(2))
                {
                    ...
                    SICStus.dbgPrintln("Created " + (fromGlue ? "glue " : "") + "SPTerm " + superToString() + " nativeTermRef==" + nativeTermRef + " in Thread " + this.creating_thread.getName());
                }
            ...
            this.context = sp.query_stack; // the age of this SPTerm object, also the freelist to use
            if (this.context != null
                && this.context.term_stack_mark != null
                && ! (this.context.term_stack_mark.nativeTermRef < nativeTermRef)
                ) {
                String msg = 
                    "** ERROR: Internal Jasper Error: "
                    + "! (this.context.term_stack_mark.nativeTermRef (" + this.context.term_stack_mark.nativeTermRef
                ...
                throw new NullPointerException(msg); // Hack to avoid declaring that we can throw exceptions
            }
            if (debugging(2)) {
                SICStus.dbgPrintln("Exit: " + superToString() + ".initSPTerm(" + sp + ", " + nativeTermRef + ", " + fromGlue + ")");
            }
    ...
    void dumpTermStack()
    {
        ...
        for (x=this; x != null; x=x.next) {
            SICStus.dbgPrintln("Term " + x.superToString()
                               + " nativeTermRef=" + x.nativeTermRef
                               + " context=" + x.context);
        }
    }
    ...
    final int termAge()
    {
        // Ideally we should keep track of the age of the referred term separately.
        // The way to do this is:
        // 1. in putTerm(t): set this.termAgeTick to t.termAge()
        // 2. in putAtom, putFunctor (arity zero): set this.termAgeTick to zero (very old)
        ...
        throws IllegalTermException
    {
        ...
        if (age() < sp.age()) {
            if (debugging()) {
                SICStus.dbgPrintln("** Warning "
                                   + "putting term (age " + sp.age() + ") into SPTerm (age " + age() +")");
            }
            if (shouldCheckAge()) {
        ...
        throws IllegalTermException
    {
        ...
        if (age() < t.termAge()) {
            if (debugging()) {
                SICStus.dbgPrintln("** Warning "
                                   + "putting SPTerm (age=" + t.age() + ", termAge=" + t.termAge() + ") into SPTerm (age " + age() +")");
            }
            if (shouldCheckAge()) {
                throw new IllegalTermException(sp);
            }
    ...
    /* Comment for when it was off by default
     * <p> Experimental feature: If the system property
     * <code>se.sics.jasper.SICStus.reuseTermRefs</code> is
     * <code>true</code> then the Prolog
     * side SP_term_ref is made available for reuse.
     * A future release will make reuse the default.
     * @see se.sics.jasper.SICStus#setReuseTermRefs
     */
    /** Invalidates the SPTerm object and makes the SP_term_ref available
     *   for re-use.
        ...
        throws SPException
    {
        synchronized (sp) {
            ...
            // make Prolog term available for Prolog GC and make it [] for when it is reused.
     ...
     * 
     * @param sp Pointer to the SICStus object.
     */
     ...
     * Modifications to the created SPTerm does not affect the input
     * SPTerm.
     * 
     * @param sp Pointer to the SICStus object.
     * @param value The SPTerm object whose term to use
     * 
     * @see se.sics.jasper.SPTerm#putTerm
     */
    public SPTerm(SICStus sp, SPTerm value)
        throws IllegalTermException, ConversionFailedException
    {
        synchronized (sp) {
            initSPTerm(sp);
            putTerm(value);
     ...
     * 
     * @param sp Pointer to the SICStus object.
     * @param value String describing the initial value of the atom.
     * 
     * @see se.sics.jasper.SPTerm#putString
     */
    public SPTerm(SICStus sp, String value)
     ...
     * @param args The arguments as an array of SPTerm:s 
     * 
     ...
     * @param sp Pointer to the SICStus object.
     * @param functor The functor of the compound term as a string
     * @param arity The arity of the compound term (ignored)
     * @param args The arguments as an SPTerm array.
...
//      */
//     /* [PM] 3.8.5 no longer public */ SPTerm(long nativeTermRef) // Used by glue code
//     {
//         SICStus sp = SICStus.spGlobal; // [PM] should be an argument
//         // DBG
//         // SICStus.dbgPrintln("Calling new SPTerm(long) (nativeTermRef=" + nativeTermRef + ")");
...
//     }
    ...
    /**
     * Creates a term given the native term ref.  <p>
     * Used by glue code
     * @since 3.9
     * 
     */
    SPTerm(SICStus sp, long nativeTermRef) // Used by glue code
    {
        ...
        if (debugging()) SICStus.dbgPrintln("Calling new SPTerm(sp, long) (nativeTermRef=" + nativeTermRef + ")");
...
//      * technical reasons it must be public. (No longer public as of SP 3.8.5) <p>
//      * 
//      * <i>The use of this constructor is strongly discouraged!</i>
//      */
//     /* [PM] 3.8.5 no longer public */ SPTerm()  // [PM] Called by glue for -term,-atom
//     {
...
//     }
    ...
    /**
     * 3.8.6 SICStus constructor calls this to create a faked SPTerm
     * that terminates the term_stack. (SPRM 2063)
     * <p>
     * 
     */
     ...
     * SPTerm object becomes invalid as the result of of explicit
     * {@link #delete delete}, closing,
    ...
    /**
     * As isValid(). In addition verifies that this SPTerm belongs to
     * the SICStus  object argument.
     */
    ...
    /*public*/ long GetNativeTermRef()
        throws IllegalTermException
    {
        // SICStus.dbgPrintln("GetNativeTermRef (" + nativeTermRef + ") isValid: " + isValid());
        synchronized (sp) {
            CheckValid();
    ...
    {
      if (!isValid()) {
          if (debugging()) SICStus.dbgPrintln("!" + superToString() + ".isValid, this.sp==" + this.sp + ", nativeTermRef==" + this.nativeTermRef);
    ...
    {
      if (!isValid(sp)) {
          if (debugging()) SICStus.dbgPrintln("!" + superToString() + ".isValid(" + sp + "), this.sp==" + this.sp + ", nativeTermRef==" + this.nativeTermRef);
          throw new IllegalTermException(this.sp);
      }
    }

    /**
     * Calls isValid() (which see) and
     * legalCallerOrThrowNullPointerException (which also see).
     * All public methods should start with calling this method.
     */
    ...
    {
        if (debugging(2)) {
            SICStus.dbgPrintln("Invalidate " + (fromGlue ? "glue" : "") + "SPTerm " + super.toString() + " nativeTermRef==" + nativeTermRef + " in " + Thread.currentThread()
                               + ((this.creating_thread == Thread.currentThread())
                                  ? 
                                  "" 
                                  :
                                  ...
                                  + 
                                  ( this.creating_thread!=null ? this.creating_thread.getName() : "null!?(ERROR)" )
                                  )
                               );
    ...
    /** 
     * Returns the type of this term. The return value is as defined by the constants
     * <code>SP_TYPE_INTEGER</code>, <code>SP_TYPE_FLOAT</code>,
     * <code>SP_TYPE_ATOM</code>, <code>SP_TYPE_VARIABLE</code>,
     * and <code>SP_TYPE_COMPOUND</code> 
     *
     ...
     * @return <code>true</code> if the term is a variable; <code>false</code> otherwise
     */
     ...
     * @return <code>true</code> if the term is an integer; <code>false</code> otherwise
     */
     ...
     * @return <code>true</code> if the term is an atom; <code>false</code> otherwise
     */
     ...
     * @return <code>true</code> if the term is a float; <code>false</code> otherwise
     */
     ...
     * Equivalent to the Prolog code <code>isList(T) :- nonvar(T), T=[_|_].</code>
     * @return <code>true</code> if the term is a list cell; <code>false</code> otherwise
     */
     ...
     * @return <code>true</code> if the term is the atom <code>[]</code>; <code>false</code> otherwise
     */
    ...
    //              }
    //              catch (SPException e) {
    //                  if (debugging()) SICStus.dbgPrintln("** Warning Exception in isEmptyList");
    //                  return false;
    //              }
    //          }
    //          return false;
    ...
    /**
     * Tests if the term is atomic. A term is atomic if it is 
    ...
    /**
     * Tests if the term is a number. A number is a term instantiated 
     * to an integer or a float.
     * @return <code>true</code> if the term is a number; <code>false</code> otherwise
     */
     ...
     * @exception se.sics.jasper.ConversionFailedException The term could not be
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutInteger(nativeTermRef, value); }
            catch ( NativeCodeException nce )
     ...
     * converted to a integer.
     * @since 3.9.1
     * FIXME: Should be made public when Java longs (64bit) can be put also on 32bit machines. /FIXED [PD] 3.9.2
     * Added in order to support 64bit platforms (Tru64)
     */
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutFloat(nativeTermRef, value); }
            catch ( NativeCodeException nce )
...
//            CheckValid();
     ...
     *
     * @param value The value of the atom as a String
     * @return Returns a reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to an atom.
     */  
    public SPTerm putString(String value)
        throws ConversionFailedException, IllegalTermException
    {
        synchronized (sp) { 
// [PD] 3.9.2
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutString(nativeTermRef, value); }
            catch ( NativeCodeException nce )
     ...
     * representation of a Prolog atom represented as a Java
     * <code>long</code>.
     *
     * @param canonical_value The canonical representation of an atom.
     * @return Returns a reference to itself.  
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to an atom.
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutAtom(nativeTermRef, canonical_value); }
            catch ( NativeCodeException nce ) 
    ...
    /** 
     * Assigns the term to a Prolog atom, given the canonical representation
     * of the atom as an SPCanonicalAtom object.
     *
     * @param cAtom The canonical representation of an atom.
     * @return Returns a reference to itself.  
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutString(nativeTermRef, cAtom.getString()); }
            catch ( NativeCodeException nce ) 
                { throw new ConversionFailedException(sp, "to atom (canonical)"); }
            return this;
        }
     ...
     * @return Returns a reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to a Java object reference.
     */
    public SPTerm putObject(Object obj)
        throws ConversionFailedException, IllegalTermException
    {
        synchronized (sp) {
...
//            CheckValid();
    ...
    /**
     * Returns the object encapsulated in the Prolog term. This method 
     * should only be called on terms to which a Java object has been assigned 
     * using the method {@link se.sics.jasper.SPTerm#putObject}.
     * @return A Java object
     */
    public Object getObject()
        throws NativeCodeException, IllegalTermException
    {
        synchronized (sp) { 
// [PD] 3.9.2
//            CheckValid();
     ...
     * 
     * @param new_term The term's new value
     * @return Returns a reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to another term.
     */
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutTerm(this.nativeTermRef, new_term.nativeTermRef); }
            catch ( NativeCodeException nce )
    ...
    /** 
     * Assigns the term to a list of character codes from a Java <code>String</code>.
     * 
     * @param string The String object
     * @return Returns a reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to list of characters. 
     */
    public SPTerm putListChars(String string)
        throws ConversionFailedException, IllegalTermException
    {
        synchronized (sp) { 
// [PD] 3.9.2
//            CheckValid();
            ...
            try { sp.spPutListChars(this.nativeTermRef, string); }
            catch ( NativeCodeException nce )
     ...
     * 
     * @param string The string containing the printed representation of the number.
     * @return Returns a reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to number.
     */
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutNumberChars(this.nativeTermRef, string); }
            catch ( NativeCodeException nce )
     ...
     * @param arity The term's arity
     * @return Returns a reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to compound term.
     */
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutFunctor(nativeTermRef, sp.spGetAtomFromString(functor), arity); }
            catch (NativeCodeException nce)
...
//            CheckValid();
     ...
     * 
     * @param functor The term's functor as a Java <code>String</code>
     * @param arity The term's arity (ignored)
     * @param args The arguments of the term
     * @return Returns a reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to compound term.
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spPutList(nativeTermRef); }
            catch ( NativeCodeException nce )
...
//            CheckValid();
            ...
            try { sp.spConsList(nativeTermRef, head.nativeTermRef, tail.nativeTermRef); }
            catch ( NativeCodeException nce )
    ...
    /* Methods for accessing SPTerms. Corresponds more or less directly to
     * the SP_get_xxx() functions in the C-Prolog interface.  */
...
//            CheckValid();
     ...
     *
     * @return The canonical representation of a Prolog atom as a Java <code>long</code>.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to the canonical representation of an atom.
     * @deprecated Use getCanonicalAtom instead.
     */
    /* public */ /* Deprecated in 3.8.5, no longer piblic in 3.9 */
    long getAtom()       // [PM] 3.8.5 also used by glue, must have the same name as in SPCanonicalAtom (but need not be public)
        throws ConversionFailedException, IllegalTermException
    {
        synchronized (sp) {
// [PD] 3.9.2
//            CheckValid();
     ...
     * Obtains the canonical representation of a Prolog atom.
     *
     * @return The canonical representation of a Prolog atom as an SPCanonicalAtom object.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to the canonical representation of an atom.
     * @exception se.sics.jasper.AtomRegisterFailure
     * @see se.sics.jasper.SPCanonicalAtom
     */
...
//            CheckValid();
     ...
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to a list of characters.
     */
...
//            CheckValid();
            ...
            try { return sp.spGetListChars(/*this, */nativeTermRef); }
            catch ( NativeCodeException nce )
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { return sp.spGetNumberChars(/* this, */nativeTermRef); }
            catch ( NativeCodeException nce )
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { return sp.spGetStringFromAtom(sp.spGetFunctorCanonical(this.nativeTermRef)); }
            catch ( NativeCodeException nce )
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { return sp.spGetFunctorArity(nativeTermRef); }
            catch ( NativeCodeException nce )
     ...
     * 
     * @param head The term which will be assigned the head
     * @param tail The term which will be assigned the tail
     *
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spGetList(this.nativeTermRef, ((SPTerm)head).nativeTermRef, ((SPTerm)tail).nativeTermRef); }
            catch ( NativeCodeException nce )
    ...
    public Term getArg(Integer i, Term arg)
        throws ConversionFailedException, IllegalTermException
    {
     ...
     * @param arg The term to which the <code>i</code><i>th</i> argument will be
     * assigned.
     * @return A reference to itself.
     * @exception se.sics.jasper.ConversionFailedException The term could not be
     * converted to the argument of a compound term.
     */
...
//            CheckValid();
            ...
            // [PM] 3.8.5 sp.checkLegalCaller();
            try { sp.spGetArg(i, this.nativeTermRef, ((SPTerm)arg).nativeTermRef); }
            catch ( NativeCodeException nce )
     ...
     * @param with The term with which to unify.
...
//            CheckValid();
     ...
     * @param with The term to compare with.
...
//            CheckValid();
     ...
     * sophisticated, for example, it does not take operators or lists
     * into account.
     */
    ...
    String toStringRec(int level)
    {
        ...
        synchronized (sp) {
            ...
            //
            //     if (level > 20) {
            //         SICStus.dbgPrintln("Warning: SPTerm::toString() excessive recursion (" + level + ")");
            //         return "...";
            //     }
            // }
            ...
            try {
                ...
                if (debugging()) sp.dbgPrintln("in SPTerm.toString(): caller is legal");
                switch (type()) // Does CheckValid()
                    ...
                    case SPTerm.SP_TYPE_ATOM:
                        if (debugging()) sp.dbgPrintln("    SP_TYPE_ATOM");
                        return getString();
    ...
    public String toString(Term options) throws SPException
    {
        ...
        if (debugging()) SICStus.dbgPrintln("Entering SPTerm.toString(Term options)");
        checkValidAndLegalCaller();
        synchronized (sp) {
            stringBuf = new StringBuffer();
            jstreamCode = sp.openStringBufferStream(stringBuf);
            try {
                if (debugging()) SICStus.dbgPrintln("SPTerm.toString(Term options): jstreamCode==" + jstreamCode);
...
//                if (debugging()) SICStus.dbgPrintln("toString(): varMap==" + varMap);
...
//         if (debugging()) SICStus.dbgPrintln("Enter: " + superToString() + ".toString(" + options + ")");
//         synchronized (sp) {
//             stringBuf = new StringBuffer();
//             jstreamCode = sp.openStringBufferStream(stringBuf);
//             if (debugging()) SICStus.dbgPrintln("toString(): jstreamCode==" + jstreamCode);
//             try {
...
//     }
    ...
    /**
     * Converts a list to an array of SPTerm. Useful when examining
     * lists returned from Prolog queries.
     *
     * This is of course only possible on a list.
     * 
     * @exception se.sics.jasper.ConversionFailedException If the term
     * could not be converted to a list.
...
//            CheckValid();
            ...
            // [PM] 3.8.5 Always documented as throwing ConversionFailedException so why catch it?
            // try
            {   
