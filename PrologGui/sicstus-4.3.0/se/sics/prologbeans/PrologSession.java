...
public class PrologSession {
	...
	/**
	 * Sets the session id for this prolog session. If the id is set each query
	 * sent to the prolog will include this session id. Note: the session id can
	 * only be set once per session and can not be changed.
	 * 
	 * @param id
	 ...
	 * given name for this method to work.} {@if.dotnet This method can only be
	 * used in Prologbeans/Java. If called in .NET it will throw an Error.}
	 * 
	 * @param name
	 ...
	 * 
	 * An example:<br>
	 * </br> <code>QueryAnswer answer = executeQuery("evaluate(In,Out)",
