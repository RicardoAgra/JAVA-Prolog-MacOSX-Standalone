...
fzn_run_stream(Stream, Options, GoalArg) :-
	...
	fzn_new_counter(Found),
	state_set([found,solutions,search], State0, [Found,Solutions,Search], State1),
	call_cleanup((   fzn_timeout(Options1,
                                     (   call_cleanup(error_handler(interpret_items(Stream, State1,
                                                                                    State), Stream),
			 ...
			 write_exhausted(State, Exhausted, TimeOut, FoundV),
			 write_statistics(Options1, Statistics0, Statistics1,
					  Statistics2, FoundV, TimeOut)
		     ),
		     (   dispose(Found),
...
fzn_timeout(Options, Goal, TimeOut) :-
	memberchk(timeout(T), Options), !,
	time_out(Goal, T, TimeOut).
fzn_timeout(_, Goal, success) :-
	call(Goal).
...
write_exhausted(_, _, time_out, SolutionsFound) :- !,
	(   SolutionsFound = 0
	->  write('=====UNKNOWN====='), nl
	;   true
	).
write_exhausted(_, true, _, SolutionsFound) :- !,
	(   SolutionsFound = 0
	->  write('=====UNSATISFIABLE=====')
	;   write('==========')
	),
	nl.
write_exhausted(_, _, _, _).

write_statistics(Options, S0, S1, S2, SolutionsFound, TimeOut) :-
	memberchk(statistics(true), Options), !,
	nl,
	(   TimeOut = time_out
	->  write('% time out!'), nl
...
fzn_all_solutions(State, Exhausted) :- 
	state_get([found,solve,solutions], State, [Found,Solve,Solutions]),
	fzn_solve(State),
	integer(Solutions),
	(   fzn_get_objective(Solve, _)
	->  Exhausted = true
...
fzn_new_counter(Counter) :-
	new_integer(0, Counter).
...
:- public fzn_increment_counter/1.
fzn_increment_counter(Counter) :-
...
fzn_post(State) :-
	state_get([model,solve], State, [Model,Solve]),
	term_variables_bag(Solve, Vars),
...
fzn_output(State) :-
	...
	call(Goal),
	flush_output.		% hakanm reported apparently annoying buffering
...
% [PM] 4.2.1 Unused
% table_lookup_type(Id, Table, Type) :-
%         table_lookup(Id, Table, _, Type).

insert_var(X, [], [X]) :- !.
insert_var(X, [Y|Xs], [Y|Xs]) :-
	X == Y, !.
insert_var(X, [Y|Xs], [Y|Ys]) :-
	insert_var(X, Xs, Ys).

insert_vars([], Xs, Xs).
insert_vars([Y|Ys], Xs0, Xs) :-
	insert_var(Y, Xs0, Xs1),
	insert_vars(Ys, Xs1, Xs).
...
par_init(Par, Type, Ident, Anns, Value0, State0, RestAnns, State) :-
	...
	;   (   selectchk(var_is_introduced, Anns, Anns1)
		...
		insert_var(Value, Vars0, Vars),
		Anns1 = Anns
	    ),
	    (   selectchk(output_var, Anns1, RestAnns)
	    ->  order_insert(OutputVars0, Ident-[], OutputVars)
	    ;   OutputVars = OutputVars0,
		RestAnns = Anns1
	    )
	),
...
order_insert([], X, [X]).
order_insert([X|Xs], Y, [Y,X|Xs]) :-
	Y @=< X, !.
order_insert([X|Xs], Y, [X|Ys]) :-
	order_insert(Xs, Y, Ys).
...
fzn_constraint_clpfd(cumulative, [[S]-M, [D]-N, [R]-O, B], Anns,
		     [cumulatives(Tasks, [machine(0,B)],
				  [bound(upper),task_intervals(true)])],
		     [array(M, int), array(N, int), array(O, int), int],
		     Anns) :-
	(   foreach(Si,S),
	    foreach(Di,D),
...
fzn_constraint_clpfd(diffn_sicstus, [[X]-XN, [Y]-YN, [DX]-DXN, [DY]-DYN], Anns,
		     [disjoint2(Rectangles, [global(true)])],
...
%                       array[int] of int: w)
fzn_constraint_clpfd(bin_packing_sicstus, [Capa, [Bin]-N, [Weight]-O], Anns0,
	...
	fzn_bin_packing_clpfd(Capa, Bin, Weight, Domain, G, []).

% bin_packing_capa(array[int] of int: capa,
%                  array[int] of var int: bin,
% 		   array[int] of int: weight)
fzn_constraint_clpfd(bin_packing_capa_sicstus, [[Capa]-M, [Bin]-N, [Weight]-O, MinIndex], Anns0,
	...
	fzn_bin_packing_capa_clpfd(Capa, Bin, Weight, MinIndex, Domain, G, []).

% bin_packing_load(array[int] of var int: load,
%                  array[int] of var int: bin,
% 		   array[int] of int: weight)
fzn_constraint_clpfd(bin_packing_load_sicstus, [[Load]-M, [Bin]-N, [Weight]-O, MinIndex], Anns0,
		     G, [array(M, int), array(N, int), array(O, int), int], Anns) :-
	(   select(domain, Anns0, Anns)
	->  Domain = true
	;   Anns0 = Anns,
	...
	fzn_bin_packing_load_clpfd(Load, Bin, Weight, MinIndex, Domain, G, []).
...
fzn_bin_packing_load_clpfd(Load, Bin, Weight, MinIndex, Domain) -->
	{   foreach(L,Load),
	    foreach(Min,Mins),
	    foreach(Max,Maxs)
	do  fd_min(L, Min),
	...
	{min_member(MinL, Mins)},
	{max_member(MaxL, Maxs)},
	{Diff is MaxL-MinL},
	{NDiff is MinL-MaxL},
	{length(Load, M)},
	{length(Bin, N)},
	...
	{   foreach(X,Bin),
	    foreach(W,Weight),
	    foreach(task(X,1,_,W,0),ItemTasks)
	do  true
	},
	(   foreach(L1,Load),
	    fromto(MinTasks,[task(I,1,_,MinSlack,0)|MinTasks2],MinTasks2,ItemTasks),
	    fromto(MaxTasks,[task(I,1,_,MaxSlack,0)|MaxTasks2],MaxTasks2,ItemTasks),
	    for(I,MinIndex,MaxIndex),
	    param(Diff,NDiff,MinL,MaxL/*,Bin,Weight*/)
	do  [MinSlack #= MinL-L1],
	    [MaxSlack #= MaxL-L1],
	    {MinSlack in NDiff..0},
	    {MaxSlack in 0..Diff}
	),
	{sumlist(Weight, Weights)},
	[sum(Load, #=, Weights)],
	{   Domain=false
	->  Options=[]
	;   Options=[generalization(true),task_intervals(true)]
	},
	[cumulatives(MinTasks, [machine(0,MinL)], [bound(lower)|Options])],
	[cumulatives(MaxTasks, [machine(0,MaxL)], [bound(upper)|Options])].

fzn_bin_packing_capa_clpfd(Capa, Bin, Weight, MinIndex, Domain) -->
	{length(Capa, M)},
	{length(Bin, N)},
	{length(Weight, N)},
	{MaxIndex is MinIndex+M-1},
	[domain(Bin, MinIndex, MaxIndex)],
	{max_member(MaxC, Capa)},
	{   foreach(X,Bin),
	    foreach(W,Weight),
	    fromto(Tasks,[task(X,1,_,W,0)|T1],T1,T2)
	do  true
	},
	{   foreach(C,Capa),
	    fromto(T2,T3,T4,[]),
	...
	do  Slack is MaxC-C,
	    (   Slack=:=0 -> T3 = T4
	    ;   T3 = [task(I,1,_,Slack,0)|T4]
	    )
	},
				% [MC] 4.3: cumulative/2 does not prune heights
	% [cumulative(Tasks, [limit(MaxC),global(true)])],
	{   Domain=false
	->  Options=[]
	;   Options=[generalization(true),task_intervals(true)]
	},
	[cumulatives(Tasks, [machine(0,MaxC)], [bound(upper)|Options])].

fzn_bin_packing_clpfd(Capa, Bin, Weight, Domain) -->
	{   foreach(X,Bin),
	    foreach(W,Weight),
	    foreach(task(X,1,_,W,0),Tasks)
	do  true
	},
				% [MC] 4.3: cumulative/2 does not prune heights
	% [cumulative(Tasks, [limit(Capa),global(true)])].
	{   Domain=false
	->  Options=[]
	;   Options=[generalization(true),task_intervals(true)]
	},
	[cumulatives(Tasks, [machine(0,Capa)], [bound(upper)|Options])].
...
interpret_solve(satisfy, Anns, State0, RestAnns, State) :-
	state_get([found], State0, [Found]),
 	fzn_search_clpfd(Anns, State0, [], RestAnns, SubGoals, []),
	(   Found = []
	->  Increment = true
	;   Increment = fzn_increment_counter(Found)
	),
	Goal = (solve(satisfy,SubGoals), fzn_output(State0), Increment),
	state_set([solve], State0, [Goal], State).
interpret_solve(minimize(Expr0), Anns, State0, RestAnns, State) :-
	state_get([table, found, solutions, search], State0,
		  [Table0, Found, Solutions, Search]),
	(Solutions = all -> Flag = all ; Flag = best),
	type(Expr0, Table0, int),
	deref(Expr0, Table0, Expr),
 	fzn_search_clpfd(Anns, State0, Expr, RestAnns, SubGoals, []),
	(   Found = []
	->  Increment = true
	;   Increment = fzn_increment_counter(Found)
	),
	Goal = (solve([minimize(Expr),Flag,Search],SubGoals), fzn_output(State0), Increment),
	state_set([solve], State0, [Goal], State).
interpret_solve(maximize(Expr0), Anns, State0, RestAnns, State) :-
	...
	Goal = (solve([maximize(Expr),Flag,Search],SubGoals), fzn_output(State0), Increment),
	state_set([solve], State0, [Goal], State).

% this version places objective last
% fzn_search_clpfd(Anns, State0, Precious, RestAnns) -->
% 	{search_anns(Anns, State0, RestAnns, SubGoals, [])},
% 	{state_get([vars], State0, [VarBag])},
% 	{sort(VarBag, VarSet)},
% 	{term_variables_set(SubGoals, LabeledSet)},
% 	{ord_subtract(VarSet, LabeledSet, RestSet0)},
% 	{ord_del_element(RestSet0, Precious, RestSet)},
% 	(   foreach(SubGoal,SubGoals)
% 	do  [SubGoal]
% 	),
% 	({RestSet = []} -> [] ; [labeling([ff,bisect],RestSet)]), % add omitted, if any
% 	({nonvar(Precious)} -> [] ; [indomain(Precious)]). % add objective, if any
% this version doesn't place objective last
fzn_search_clpfd(Anns, State0, _Precious, RestAnns) -->
	{search_anns(Anns, State0, RestAnns, SubGoals, [])},
	{state_get([vars], State0, [VarBag])},
	{sort(VarBag, VarSet)},
	{term_variables_set(SubGoals, LabeledSet)},
...
search_ann(Ann, State0) -->
	...
	[labeling(Options, Xs)].
search_ann(labelling_ff, State0) -->
