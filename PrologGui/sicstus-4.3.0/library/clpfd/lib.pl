...
element(Index, Xs, Value) :-
	...
	;   ground(Xs) ->
	    fd_global(Goal, f(Index-IndexM,Value-ValueM,Xs,N,1,_Handle,0),
		      [dom(Index),dom(Value)])
...
scalar_product(Cs, Xs, Rel, S, Options) :-
	...
	;   scalar_op_fast(Vec, Sum, Op0, Op),
	    fd_global(Goal4, state(Vec,Op,Sum,0/*Nground*/,_,0), Susp)
	),
	% (   var(Op) -> true
...
dc_linear(Cs, Xs, S, Vec, Sum, _Goal) :-
	(   foreach(f(_,B,_),Vec),
	    foreach(none(B),Susp),
	    count(_,1,N)
	do  true
	),
	fd_global(dc_linear(Cs,Xs,S), state(Vec,N,Sum,N/*Ntargets*/,_,0), Susp).
...
eq_iff(X, Y, B) :-
	...
	;   fd_global(Goal, state(X,XMut,Y,YMut,B,BMut,_Handle,0), [none(X),none(Y),none(B)])
	).
...
formula_to_case(Formula1, Module, Expanded) :-
	...
	;   Expanded = case(Template,Tuples,Dag,[]),
