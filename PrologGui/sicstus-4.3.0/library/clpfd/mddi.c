...
static int
mddi_propagate(Wam wam, struct mddi_data *pdata)
{
  int ent = -1;
  int tuple, i, j;
  int nvars = pdata->nvars;
  ...
  while (tuple != -1) {
	...
	SP_globref ref = ATTRIBUTE_LOC(tuple,i);
	Dvar dv = DVAR(i);
	    ...
	    TAGGED global)
{
  ...
  if ((pdata->entailed[tuple] & 0x3) == EDGE_ALIVE) { /* if tuple not entailed */
    ...
    if (tdata->nonground.status[col] & EDGE_ALIVE) { /* could have been processed already */
      SP_globref ref = ATTRIBUTE_LOC(tuple,col);
      Dvar dv = DVAR(col);
      SP_BOOL buried;
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
	...
	if (support) {
      ...
      dvar_fix_set(dvars+i, fdcons_set(&cons));
    }
    ...
    for (i=0; i<nvars; i++) {
      FDITER it;
      fditer_init(&it, dvar_set(DVAR(i)));
      while (!fditer_empty(&it)) {
	TAGGED y = fditer_next(&it);
  ...
  /* RESUME */
  if (posted) {
    for (i=0; i<nvars && (pdata->entailed[0] & EDGE_ALIVE); i++) {
      SP_globref ref = ATTRIBUTE_LOC(0,i);
      Dvar dv = DVAR(i);
