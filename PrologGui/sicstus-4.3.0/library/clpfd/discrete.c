			    ...
			    int flags)
{
  ...
  for (i=0; i<nbest; i++) {
	...
	(event+k)->key = maxb = b<<3;	/* LCT event */
	...
	if (m > dur(t))
	  m = dur(t);
	if (a >= lst(t)) {
	  SP_integer key = ((a+m)<<3)+0x2; /* PLATEAU minoverlap event, a >= lst(t) */
	  ...
	  SP_integer key = ((lst(t))<<3)+0x1; /* SLOPE minoverlap event, a < lst(t) */
	  if (key < maxb) {
	    (event+k)->key = key;
	    (event+k)->value.task = t;
	    k++;
	  }
	  key = ((lst(t)+m)<<3)+0x2; /* PLATEAU minoverlap event, a < lst(t) */
	  if (key < maxb) {
	    (event+k)->key = key;
	    (event+k)->value.task = t;
	    k++;
	...
	if (m > dur(t))
	  m = dur(t);
	if (a >= est(t)) {
	  SP_integer key = ((a+m)<<3)+0x4; /* PLATEAU maxoverlap event, a >= est(t) */
	  ...
	  SP_integer key = ((est(t))<<3)+0x3; /* SLOPE maxoverlap event, a < est(t) */
	  if (key < maxb) {
	    (event+k)->key = key;
	    (event+k)->value.task = t;
	    k++;
	  }
	  key = ((est(t)+m)<<3)+0x4; /* PLATEAU maxoverlap event, a < est(t) */
	  if (key < maxb) {
	    (event+k)->key = key;
	    (event+k)->value.task = t;
	    k++;
		   ...
		   SP_integer end)
{
  ...
  for (i=0, j=0; i<nbtasks; i++) {
	...
	est(t) < end &&
	lct(t) > start) {
      event[j].key = ((est(t) > start ? est(t) : start)<<1)+1;
      event[j].value.task = t;
      event[j+1].key = ((lct(t) < end ? lct(t) : end)<<1);
			     ...
			     Dvar limitvar)
{
  ...
  for (i=0; i<nbtasks-1; i++) {
	...
	while (pdata->cp[e1+1].start < b) {
	  ...
	  if (total<=limtu)
	    goto noedge;
	  pdata->event[nbev0].key = (pdata->cp[e1+1].start<<1)+(delta<0);
	  pdata->event[nbev0++].value.delta = delta;
	  e1++;
	}
	nbev = nbev0;
	if (tcpa<tcpb) {
	  pdata->event[nbev].key = (tcpa<<1)+1;
	  pdata->event[nbev++].value.delta = -res(t);
	  pdata->event[nbev].key = (tcpb<<1);
	  pdata->event[nbev++].value.delta = res(t);
	}
	if (ucpa<ucpb) {
	  pdata->event[nbev].key = (ucpa<<1)+1;
	  pdata->event[nbev++].value.delta = -res(u);
	  pdata->event[nbev].key = (ucpb<<1);
	  pdata->event[nbev++].value.delta = res(u);
	}
	if (nbev0 < nbev) {
	  qsort_asc_event_ptrs(wam, pdata,nbev);
...
/*    X(0) = RefTerm(State0); */
  ...
  do {
    ...
    if (unary_filtering(wam, tasks,j,0,(flags&0x8)>>2,NULL,NULL,0,&ignore,&ignore,size_mem) >= 0x2)
      goto ret;
				    ...
				    int nbdiffs)
{
  ...
  for (k=0; k<nbdiffs; k++) {
    ...
    j = (int)pdata->id_map[(dc+k)->sj];
    if (i>=0 && j>=0) {
      if (dvar_min_l(diffvar+k) > (enable(tasks+j)<2 ? 0 : -dur(tasks+j))) {
	nbprec++;
	if (!add_precedence(pdata,i,j,precedence)) {
	  rc = 2; goto ret;
	}
      }
      if (dvar_max_l(diffvar+k) < (enable(tasks+i)<2 ? 0 : dur(tasks+i))) {
	nbprec++;
