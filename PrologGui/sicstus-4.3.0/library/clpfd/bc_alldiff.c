...
/*
  ...
  State = state(f(B,Vset,XVec,As,C,NGround),Handle,Stamp)
*/
void SPCDECL
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata->nvars = nvars;
    pdata->nrefs = 2*nvars+2;
    pdata->refbase = SP_alloc_globrefs(2*nvars+2);
    ptr = (char *)(pdata+1);
    pdata->dvar = (Dvar)ptr;
    ptr = (char *)(pdata->dvar+nvars+1);
    pdata->a = (SP_WORD *)ptr;
    ...
    DerefArg(tvec,X(0),1);
    fd_get_var_and_attr(tvec,RefAttr(nvars));
    // State = state(f(B,V,XVec,As,C),Handle,Stamp)
    DerefArg(tvec,X(0),5);
  ...
  {
    Dvar dv = DVAR(nvars);
    dvar_init(dv, RefAttr(nvars), RefVar(nvars));
    b = (int)dvar_min_l(dv);		/* just a counter */
  }
  for (i=n; i<nvars; i++) {
    int j = SV(i);
    Dvar dv = DVAR(j);
  ...
  /* partition into target/source */
  {
    ...
    ent = (inf<=1) && dvar_is_integer(DVAR(nvars));
  }
 ret:
  if (ent==1)
    Pfree;
