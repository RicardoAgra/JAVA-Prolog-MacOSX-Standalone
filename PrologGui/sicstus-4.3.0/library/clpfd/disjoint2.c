...
static SP_BOOL feasible_minwit(Wam wam,
				   int js, SP_integer next_start,int nsources,
				   ITEM titem,
				   SP_integer *dur,SP_integer *est,SP_integer *lct,int k)
{
  ...
  while (next_start <= ub) {
    ...
    if (wit >= pdata->event.start[1-k][ev]-dur[1-k] && wit <= pdata->event.end[1-k][ev])
      return FALSE;
    js = next_start2d(wam, js+1,nsources,titem,dur,est,lct,ub,ub,k,&next_start);
  }
  return TRUE;
}
				   ...
				   SP_integer *dur,SP_integer *est,SP_integer *lct,int k)
{
  ...
  while (next_end >= lb) {
    ...
    if (wit >= pdata->event.start[1-k][ev]-dur[1-k] && wit <= pdata->event.end[1-k][ev])
      return FALSE;
    je = prev_end2d(wam, je-1,titem,dur,est,lct,lct[k],lb,k,&next_end);
  }
  return TRUE;
}
			    ...
			    SP_BOOL *change)
{
  ...
  for (i=0, nt=0; i<nsources; i++) {
    ...
    if (!incremental ||
	(flags & 0x8) ||	/* with margins, titem may have lost
				   STATUS_TARGET property in a previous
				   iteration [MC] 3.9.2 */
...
*/
static SP_BOOL disjoint2_check(Wam wam, 
			       int nsources,
			       SP_BOOL incremental)
{
  struct disjoint2_data *pdata = fd.gdata;
...
#if DECOMPOSITION
static void decompose2d(Wam wam,
			int nsources, int ntargets, SP_BOOL incremental)
{
  struct disjoint2_data *pdata = fd.gdata;
...
/*    X(0) = RefTerm(State0); */
  ...
  if (state_stamp != pdata->stamp) { /* trust nothing */
	...
	for (k=0; k<2; k++) {
	  ...
	  dvar_init(DVAR2D(it,k), ref, ref+1);
  ...
  if (!(incremental && (flags & 0x20)) && /* not in the incremental case */
      !(flags & 0xe)) {		/* not applicable with margins or wrap-around */
    ...
    if (j>0 && !disjoint2_check(wam, j,incremental))
      goto ret;
  }
#endif
  ...
  if (pruned1==TERMIN) {
    ...
    /* special case of first part of disjoint2_filter(wam, ) */
    SP_integer start[2], end[2], dur_threshold[2];
    ...
    if (tix>0) {
      ...
      if (j>0)
	if (!disjoint2_filter(wam, j,target0,tix,incremental,&change))
	  goto ret;
      pdata->iteration++;
    }
  }
  ...
  for (; change; pdata->iteration++) {
    ...
    if (!(flags & 0x8)) { /* no margins */
      j = collect_source2d(wam, TRUE);
      if (j>0)
	if (!disjoint2_filter(wam, j,pdata->target,pdata->ntargets,
			      (incremental || pdata->iteration>1),
			      &change))
	  goto ret;
      if (pdata->synchro_table &&
	  !wake_sweep_synchro(wam, pdata,
			      (incremental || pdata->iteration>1),
			      &change))
        goto ret;
    } else {			/* margins */
	...
	j = collect_source2d(wam, TRUE);
	if (j>0)
	  if (!disjoint2_filter(wam, j,pdata->target+ti,1,
				(incremental || pdata->iteration>1),
				&change))
	    goto ret;
      }
  ...
  decompose2d(wam, j,nactive_items,(incremental && (flags & 0x20)));
