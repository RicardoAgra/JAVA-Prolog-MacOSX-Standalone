		 ...
		 struct sorting_data *pdata)
{
  ...
  while (donormal) {
    ...
    for (i=1; i<ntargets; i++) {
      int y = YV(i);
      if (FDlt(YMIN(y),cur) && dvar_fix_min_t(YVAR(y),cur)<0)
	return FALSE;
      cur = YMIN(y);
    }
    /* normalize max(Y) */
    ...
    for (i=ntargets-1; i>=0; i--) {
      int y = YV(i);
      if (FDgt(YMAX(y),cur) && dvar_fix_max_t(YVAR(y),cur)<0)
	return FALSE;
      cur = YMAX(y);
    }
    donormal = FALSE;
	...
	return FALSE;
      if (PMIN(x)<y+1) {
	if (dvar_fix_min_l(PVAR(x),y+1)<0)
	  return FALSE;
	if (PMIN(x)>y+1)
	  donormal = TRUE;
      }
      xlb = YMIN(PMIN(x)-1);
      if (FDlt(xmin,xlb)) {
	if (dvar_fix_min_t(XVAR(x),xlb)<0)
	  return FALSE;
	xmin = XMIN(x);
	if (FDgt(xmin,xlb))
	  donormal = TRUE;
	...
	return FALSE;
      if (PMAX(x)>y+1) {
	if (dvar_fix_max_l(PVAR(x),y+1)<0)
	  return FALSE;
	if (PMAX(x)<y+1)
	  donormal = TRUE;
      }
      xub = YMAX(PMAX(x)-1);
      if (FDgt(xmax,xub)) {
	if (dvar_fix_max_t(XVAR(x),xub)<0)
	  return FALSE;
	xmax = XMAX(x);
	if (FDgt(xmax,xub))
	  donormal = TRUE;
	...
	if (FDlt(YMIN(y),XMIN(x)) || FDgt(YMAX(y),xmax)) {
	  if (dvar_fix_interval_t(YVAR(y),XMIN(x),xmax)<0)
	    return FALSE;
	  donormal = TRUE;
	}
      }
...
match_up(struct sorting_data *pdata)
{
  ...
  */
  ...
  for (i=0; i<ntargets; i++) {
    y = YV(i);
    x = YMATE(y);
    rc = dvar_fix_max_t(YVAR(y),XMAX(x));
    CHECK(rc);
  }
  return TRUE;
}
...
match_down(struct sorting_data *pdata)
{
  ...
  */
  ...
  for (i=0; i<ntargets; i++) {
    y = YV(i);
    x = YMATE(y);
    rc = dvar_fix_min_t(YVAR(y),XMIN(x));
    CHECK(rc);
  }
  return TRUE;
}
...
narrow_x(struct sorting_data *pdata)
{
  ...
  for (i=0; i<ntargets; i=n) {
    ...
    while (j<n) {
      ...
      if (!FDgt(xmin,ymax)) {
	TAGGED ymin = YMIN(y);
	int rc = dvar_fix_min_t(XVAR(x),ymin);
	CHECK(rc);
	j++;
      } else
	if (++i<ntargets) {
  ...
  for (i=ntargets-1; i>=0; i=n) {
    ...
    while (j>n) {
      ...
      if (!FDlt(xmax,ymin)) {
	TAGGED ymax = YMAX(y);
	int rc = dvar_fix_max_t(XVAR(x),ymax);
	CHECK(rc);
	j--;
      } else
	if (--i>=0) {
...
narrow_p(struct sorting_data *pdata)
{
  ...
  for (i=0; i<ntargets; i=n) {
    ...
    while (j<n) {
      ...
      int pmin = (int)PMIN(x);
      if (pmin<=y+1) {
	int rc = dvar_fix_min_l(PVAR(x),y+1);
	CHECK(rc);
	j++;
      } else
	if (++i<ntargets)
  ...
  for (i=ntargets-1; i>=0; i=n) {
    ...
    while (j>n) {
      ...
      if (pmax>=y+1) {
	int rc = dvar_fix_max_l(PVAR(x),y+1);
	CHECK(rc);
	j--;
      } else
	if (--i>=0)
...
/*    X(0) = RefTerm(State0); */
  ...
  for (i=0; i<ntargets; i++) {
    int elt = XV(i);
    dvar_init(XVAR(elt), RefAttr(elt), RefVar(elt));
    dvar_init(PVAR(elt), RefAttr(elt+nvars), RefVar(elt+nvars));
    elt = YV(i);
    dvar_init(YVAR(elt), RefAttr(elt+2*nvars), RefVar(elt+2*nvars));
  }
  for (i=ntargets-1; i>=0; i--)
