...
request_tell_value(Wam wam, TAGGED value)
{
  ...
  if (fd.debugging) {
    ...
    if (why<0) {
      ...
      DerefSwitch(X(EVAL_ARITY+1),fd_told(wam, why););
...
request_tell(Wam wam, TAGGED fdset)
{
  ...
  if (fd.debugging) {
	...
	why = fd_tell(wam, fd_intersection_interval(wam, fdset,min,max)); /* GC */
      } else
	why = fd_tell_unsafe(wam, fd_intersection(wam, old,fdset)); /* GC */
    }
    if (why<0) {
      RequireHeap(2,EVAL_ARITY+2);	/* GC */
      h = w->global_top;
...
void
dvar_export_ix(Wam wam, Dvar dvar)
{
  unsigned int flags = dvar->flags;
  int why;
		   ...
		   TAGGED list_functor)
{
  ...
  DerefSwitch(var, fd_link(wam, var,list_functor,MakeStructure(h)););
}
