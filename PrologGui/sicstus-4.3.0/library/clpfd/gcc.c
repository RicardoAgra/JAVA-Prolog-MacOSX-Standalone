...
#if 0
/* reset all flows to zero among target vertices */
static void
costgcc_reset(struct gcc_graph *pdata, int b)
{
  int i, j;
  int nvals = pdata->nvals;
  int nvaltargets = pdata->nvaltargets;
  int nactive = pdata->nactive;
  
  /* reset dual values */
  for (i=0; i<nactive; i++) {
    VERTEX v = ACTIVE(i);
    DUAL(v,b) = v<=nvals ? 0 : -pdata->costmin[b];
    pdata->vertex.generation[v] = 0;
  }
  pdata->generation = 0;
...
/* Successive shortest paths algo.  [Ahuja et al., p.. 320-324]
 * The excess nodes are the value nodes with flow < limit,
 * plus the source node.
 * The deficit node is the target node.
 */
#if 0
static SP_BOOL
costgcc_opt_flow(struct gcc_graph *pdata,
		 SP_integer accum,
		 int b)
{
  int nvals = pdata->nvals;
  ...
  for (flow=0; flow<nvartargets; flow++) {
    VERTEX enode=0;		/* default excess node */
    VERTEX dnode;		/* deficit (VAR) node */
    VERTEX dad;
    SP_integer ddist;
    
    /* find excess node */
    for (; iexcess<nvaltargets; iexcess++) {
      enode = TVAL(iexcess);
      if (SFLOW(enode,b) < LIMIT(enode))
	break;
      enode = 0;
    }
				    
    /* determine shortest paths distances from the source */
    INVALIDATE_DIST;
    dnode = opt_shortest_paths(pdata, enode, n, b);
    if (dnode<0)
      return FALSE;
    /* Update duals. Note: dual(dnode) remains unchanged. */
    ddist = pdata->distrow[dnode];
    DUAL(enode,b) += ddist;
    for (i=pdata->heapsize+1; i<pdata->heapmax; i++) {
      VERTEX v = pdata->heap[i];
      DUAL(v,b) -= pdata->distrow[v] - ddist;
    }
    /* augment 1 unit of flow along the path */
#if EXPLICIT_FLOW
    if (enode!=0)
      INCFLOW(0,enode,b);
    INCFLOW(n-1,0,b);
    ...
    for (dad = DAD(dnode);
	 dad >= 0;
	 dnode = dad, dad = DAD(dnode)) {
      INCFLOW(dad,dnode,b);
      if (dnode>nvals)		/* add cost */
	accum += COST(dad,dnode,b);
      else if (dad>nvals)	/* subtract cost */
	accum -= COST(dnode,dad,b);
    }
  }
...
{
#if 0
  costgcc_reset(pdata,b);
  return (costgcc_opt_flow(pdata,accum,b) &&
...
/*    X(0) = RefTerm(State0); */
  ...
  dv = DVAR(0);
  dvar_init(dv, RefAttr(0), RefVar(0));
