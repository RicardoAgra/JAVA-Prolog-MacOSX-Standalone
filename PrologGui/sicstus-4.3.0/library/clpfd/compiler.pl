...
indexical('$VAR'(P), bool(Fun), 1, [Q-22,R-22], 2, P, Len, [], []) --> !,
	{Q is (P+1) mod 3,
	 R is (P+2) mod 3},
	emit(bool(Fun), Len).
...
emit(popj, 1) --> [70].
emit(error, 1) --> [72].
emit(bool(Fun), 2) --> [87,l(Fun)]. % 4.2.1
...
set_expression_check({S}, Set, Goal, ArgNo) :- !,
	union_expression_check(S, Set, Goal, ArgNo).
set_expression_check(A..B, Set, Goal, ArgNo) :- !,
	(   A==inf -> true
	;   A==sup -> true
	;   must_be_fd_integer(A, Goal, ArgNo)
	),
	(   B==inf -> true
	;   B==sup -> true
	;   must_be_fd_integer(B, Goal, ArgNo)
	),
	'$fd_range'(A, B, Set, 1).
...
union_expression_check(X, _, Goal, ArgNo) :- var(X), !,
	illarg(var, Goal, ArgNo, X).
union_expression_check((S1,S2), Set, Goal, ArgNo) :- !,
	union_expression_check(S1, Set1, Goal, ArgNo),
	union_expression_check(S2, Set2, Goal, ArgNo),
	'$fd_dom_union'(Set1, Set2, Set).
union_expression_check(I, Set, Goal, ArgNo) :-
	must_be_fd_integer(I, Goal, ArgNo),
	'$fd_range'(I, I, Set, 1).
...
linearize(E1*E2, M, I, I*Y) -->
	{E1 == E2}, !,
	linearize_rel(E1, #=, X, M),
	[nonlinear('x*x=y'(X,Y))].
linearize(E1*E2, M, I, I*Z) --> !,
	linearize_rel(E1, #=, X, M),
	linearize_rel(E2, #=, Y, M),
...
bool_decompose(element(I,L,Y), M, B) --> !,
	(   foreach(X,L),
	...
	bool_decompose(Expanded, M, B).
bool_decompose(table(Tuples,Ext), M, B) --> !,
...
unfoldeq_reify_quodivmodrem('x/y=z'(X,Y,Z), B, B3) -->
	['x/y=z'(X,Y1,Z), 'x\\=y'(Y,0,B1), 'x=y'(Y,Y1,B2), bool(4,B2,B1,1), bool(0,B1,B3,B)].
unfoldeq_reify_quodivmodrem('x div y=z'(X,Y,Z), B, B3) -->
	['x div y=z'(X,Y1,Z), 'x\\=y'(Y,0,B1), 'x=y'(Y,Y1,B2), bool(4,B2,B1,1), bool(0,B1,B3,B)].
unfoldeq_reify_quodivmodrem('x mod y=z'(X,Y,Z), B, B3) -->
	['x mod y=z'(X,Y1,Z), 'x\\=y'(Y,0,B1), 'x=y'(Y,Y1,B2), bool(4,B2,B1,1), bool(0,B1,B3,B)].
unfoldeq_reify_quodivmodrem('x rem y=z'(X,Y,Z), B, B3) -->
	['x rem y=z'(X,Y1,Z), 'x\\=y'(Y,0,B1), 'x=y'(Y,Y1,B2), bool(4,B2,B1,1), bool(0,B1,B3,B)].
...
bool_connect_top(V) --> {simple(V)}, !, [V=1].
bool_connect_top(not(V)) --> !, [V=0].
