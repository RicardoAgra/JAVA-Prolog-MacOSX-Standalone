...
struct symmcum_data {
  ...
  SP_integer *initial_size;
  TAGGED *initial_est;
  TAGGED *initial_lct;
  /* valid values in forward execution */
  SP_integer *previous_size;
  TAGGED *previous_est;
  TAGGED *previous_lct;
  /* either one of the two above */
  SP_integer *table_size;
  TAGGED *table_est;
  TAGGED *table_lct;
  TAGGED queue;
...
#define SETMACH(T)   dvar_set(DVMACH(T))
...
static SP_BOOL
prune_machine_value(Wam wam, TASK t, TAGGED trr)
{
  struct symmcum_data *pdata = fd.gdata;

  switch (dvar_prune_value_t(DVMACH(t),trr)) {
  case -1:
    return FALSE;
  case 0:
    return TRUE;
...
static SP_BOOL
fix_machine_value(Wam wam,
			 TASK t, SP_integer r/* real */)
{
  struct symmcum_data *pdata = fd.gdata;
...
static SP_BOOL
prune_machine_values(Wam wam, TASK t, TAGGED values)
{
  struct symmcum_data *pdata = fd.gdata;

  switch (dvar_prune_set(DVMACH(t),values)) {
  case -1:
    return FALSE;
  case 0:
    return TRUE;
...
static SP_BOOL
fix_machine_values(Wam wam, TASK t, TAGGED values)
{
  struct symmcum_data *pdata = fd.gdata;

  switch (dvar_fix_set(DVMACH(t),values)) {
  case -1:
    return FALSE;
  case 0:
    return TRUE;
...
static SP_BOOL
fix_origin_interval(Wam wam,
			   TASK t, SP_integer lb, SP_integer ub)
{
  struct symmcum_data *pdata = fd.gdata;
...
task_renormalize(Wam wam, TASK t)	/* origin + duration = end */
{
  ...
  for (;;) {
    ...
    if (!fix_origin_interval(wam, t, lb1, ub1))
      return FALSE;
  
  /* dur in min(end)-max(origin) ... max(end)-min(origin) */
    if (!fix_duration_interval(wam, t, lb2, ub2))
      return FALSE;
  
  /* end in min(origin)+min(dur) ... max(origin)-max(dur) */
    if (!fix_end_interval(wam, t, lb3, ub3))
      return FALSE;
  }
}
		       ...
		       )
{
  ...
  if ((pdata->options&OPT_ATMOST)
    ...
    if (MUST_OVERLAP(t,low,up)) {
      return prune_machine_value(wam,t,MakeSmall(REAL(r)));
    } else {
      SP_integer maxd, l2;
	...
	if (MINDUR(t)>0 &&
	  ...
	  return FALSE;
	return
	  fix_duration_interval(wam, t, MINDUR(t), maxd);
      } else if (SIZE_(t)>1) {
				...
				)
{
  ...
  if (!fix_machine_value(wam, t, REAL(r)))
    return FALSE;

  /* prune origin of t to enforce task t to overlap low..up */

  if (!fix_origin_interval(wam, t, up-MAXDUR(t), low))
    return FALSE;

  /* prune end of t to enforce task t to overlap low..up */

  if (!fix_end_interval(wam, t, up, low+MAXDUR(t)))
    return FALSE;
  ...
  if (mind>l2)
    mind = l2;
  if (!fix_duration_interval(wam, t, mind, MAXDUR(t)))
    return FALSE;
		 ...
		 SP_BOOL (*fun)(Wam, TASK, SP_integer, SP_integer))
{
  ...
  while (p && rc) {
    ...
    if (h==size) {
      if (!pruned) {
	pruned = TRUE;
	state = task_state(wam, t);
      }
      rc = (*fun)(wam, t,low,up-1);
    }
  }
  if (rc && pruned && task_state(wam, t)!=state) {
    rc = task_renormalize(wam, t);
    pdata->queue =
      fd_union_interval(wam, pdata->queue,TABLE_EST_T(t),TABLE_LCT_T(t));      
  }
  return rc;
}
	       ...
	       )
{
  ...
  for (i=0; i<pdata->top_prune; i++) {
    ...
    if (dvar_contains_value_t(DVMACH(t),trr)) {
      SP_integer previous_task_state = task_state(wam, t);
	...
	if (dvar_contains_value_t(DVMACH(t),trr)) { /* NECESSARY test */
      ...
      if (previous_task_state!=task_state(wam, t)) {
	if (!task_renormalize(wam, t))
	  return FALSE;
	pdata->queue = fd_union_interval(wam, pdata->queue,TABLE_EST_T(t),
				      TABLE_LCT_T(t));
      }
    }
  }
  ...
  while (t2!=NOTASK) {
	...
	if (!HAS_COMPULSORY_PART(t2) ||
	  ...
	  if ((pdata->options&OPT_ATMOST)
	    ...
	    SP_integer previous_task_state = task_state(wam, t2);
		...
		maxd = l2;
	      if (maxd<0)
		maxd = 0;
	      if (!fix_duration_interval(wam, t2, MINDUR(t2), maxd))
		return FALSE;
	    }

	    /* handle saturation if there was some pruning */

	    if (previous_task_state != task_state(wam, t2)) {
	      if (!task_renormalize(wam, t2))
		return FALSE;
	      pdata->queue = fd_union_interval(wam, pdata->queue,TABLE_EST_T(t2),
					    TABLE_LCT_T(t2));
	    }
	  }
	}
...
check_interval(Wam wam, SP_integer low, SP_integer up)
{
  ...
  while (!fditer_empty(&it)) {
	...
	*dest = fd_profile_update(wam, *dest,b,e,1);
	...
	if (!prune_machine_value(wam, t,trr))
	  goto ret;
	pdata->queue = fd_union_interval(wam, pdata->queue,TABLE_EST_T(t),
				      TABLE_LCT_T(t));
      }
			  ...
			  TASK t,SP_integer delta,SP_integer h)
{
  ...
  if (!fix_origin_interval(wam, t, low+T-maxd, up-T))
    return FALSE;

  /* Restrict the range of the end attribute of task t */

  if (!fix_end_interval(wam, t, low+T, up-T+maxd))
    return FALSE;

  /* Adjust the minimum duration of task t */

  if (!fix_duration_interval(wam, t, T, maxd) ||
      (LST(t)<low && !fix_duration_interval(wam, t, low-LST(t)+T, maxd)) ||
      (ECT(t)>up  && !fix_duration_interval(wam, t, ECT(t)-up+T, maxd)))
    return FALSE;
  ...
  if ((pdata->options&OPT_ATMOST)
      ? !fix_height_interval(wam, t, MINHEIGHT(t), -limit)
      : !fix_height_interval(wam, t, limit, MAXHEIGHT(t)))
    return FALSE;
			...
			TASK t,SP_integer delta,SP_integer h)
{
  ...
  if (T<=up-low) {
    ...
    if (EST(t)<=up-T && LCT(t)>=low+T && MAXDUR(t)>=T) {
      ...
      if (limit<0)
	limit = 0;
      if (!fix_duration_interval(wam, t, mind, limit))
	return FALSE;
    }
  }
		 ...
		 SP_integer low,SP_integer up,TAGGED machines)
{
  ...
  int nmachines = pdata->nmachines;
  int ntry_to_prune = 0;
  SP_integer sum_slack, slack, capr, sum_contribution, sum_contribution_i, h, inter,
  ...
  /* dichotomic search for min i such that LCTKEY(TASK(i)) > low 
    ...
    while (i<iend && MINEST(i)<up) {
      TASK t = TASK(i++);
      int overlap = dvar_compare_set(DVMACH(t),machines);
  ...
  for (i=0; i<ntry_to_prune; i++) {
    ...
    if (FLAGS(t) & STATUS_TARGET) {
      ...
      if (delta<0)
	delta = -delta;
      previous_task_state = task_state(wam, t);
	...
	if ((pdata->options&OPT_ATMOST)
	    ...
	    : sum_contribution_i<sum_slack)
	  if (!fix_machine_values(wam, t,machines) ||
	...
	if (pdata->include_in_machines[t]) {
	  ...
	  if ((pdata->options&OPT_ATMOST)
	       ...
	       : sum_contribution+pdata->min_length_inter[t]*h < sum_slack)
	    if (!prune_machine_values(wam, t,machines))
	      goto ret;
	}
      }
	...
	if (!task_renormalize(wam, t))
	  goto ret;
	pdata->queue = fd_union_interval(wam, pdata->queue,
					 TABLE_EST_T(t), TABLE_LCT_T(t));
      }
    }
  }
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    total_size =
      ...
      (13*ntasks + 2*nmachines)*sizeof(SP_integer);
    ...
    pdata->initial_size = (SP_integer *)ptr;
    ...
    pdata->previous_size = (SP_integer *)ptr;
  ...
  for (i=0; i<ntargets; i++) {
    TASK t = TASK(i);
    SP_integer state = task_state(wam, t);
 ...
 fixpoint:
  while (pdata->queue!=EmptySet) {
    ...
    if (!check_interval(wam, GetSmall(RangeMin(r)),GetSmall(RangeMax(r))))
      goto ret;
  }
  ...
  for (i=0; i<ntargets; i++) {
    TASK t = TASK(i);
    SP_uinteger flags = FLAGS(t);

    if ((flags & (SP_uinteger)~(STATUS_TARGET+STATUS_SOURCE)) > 0) {
