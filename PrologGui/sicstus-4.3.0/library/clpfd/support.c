...
int fd_dequeue(Wam wam, TAGGED *item)
{
...
#if ZINCDBG
    sp_ttyprintf("DEQUEUE: queues %d..%d %d..%d %d..%d %d..%d\n",
		 cur->queue[0].first,cur->queue[0].last,
		 cur->queue[1].first,cur->queue[1].last,
		 cur->queue[2].first,cur->queue[2].last,
		 cur->queue[3].first,cur->queue[3].last);
    sp_ttyprintf("DEQUEUE: event\n");
    sp_ttyflush();
#endif
    return 3;
  }
  for (i=cur->hint; i<FD_QUEUE_DAEMON; i++) {
	...
	int arity;
        ...
        type = (i==0 ? "indexical" : "global");
	tmp = TagToHeadfunctor(CTagToArg(X(0),2));
	arity = Arity(tmp);
	name = SP_string_from_atom(tmp);
...
#if ZINCDBG
  sp_ttyprintf("DEQUEUE: empty\n");
  sp_ttyflush();
#endif
  return 0;
}

static void gc_suspensions(Wam wam,
			   int index,
			   TAGGED lists_loc,
			   TAGGED tail)
{
  ...
  if (lists < w->global_uncond) { /* can't smash */
    ...
    lists = h;
    h += 8;
    FD_UPDATE_MUTABLE(MakeStructure(lists),lists_loc);
  }
  target = &lists[index];
  while (list!=tail) {
    item = CTagToCar(list);
...
static void fd_enqueue_list_gc(Wam wam,
			       int index, TAGGED lists_loc)
{
  TAGGED *lists = TagToArg(RefMutable(lists_loc),0);
  TAGGED list, tail, item, status, functor;
  struct propagator *cur = fd.current_propagator;
  int ndead=0;
  ...
  if (ndead>0)			/* there exist entailed entries */
    gc_suspensions(wam, index, lists_loc, tail);
}

static void fd_enqueue_list(Wam wam, int index, TAGGED lists)
{
  TAGGED list, item, status, functor;
  struct propagator *cur = fd.current_propagator;
  int bin = (index==3 ? 0x3 : 0x2);
  DECL_UPDATE_MUTABLE;
...
static int fd_enqueue_val(Wam wam,
			  int index,
			  TAGGED lists,
			  TAGGED *stack)
{
...
void fd_enqueue_all(Wam wam, int bits, TAGGED lists_loc)
{
  ...
  nsusp = GetSmall_int(t1);
  RequireHeap1((nsusp<<1)+8,lists_loc,EVAL_ARITY); /* 2 * #suspensions + lists/7 */
  if (bits & MASK_SINGLETON) {
    ...
    if (bits & MASK_DOM)
      fd_enqueue_list(wam, 3, lists);
    if (bits & MASK_MIN)
      fd_enqueue_list(wam, 4, lists);
    if (bits & MASK_MAX)
      fd_enqueue_list(wam, 5, lists);
    if (bits & MASK_MINMAX)
      fd_enqueue_list(wam, 6, lists);
    if (bits & MASK_VAL)
      n = fd_enqueue_val(wam, 7, lists, &X(3));
    if (n>0) {
    ...
    if (bits & MASK_MINMAX)
      fd_enqueue_list_gc(wam, 6, lists_loc);
    if (bits & MASK_MAX)
      fd_enqueue_list_gc(wam, 5, lists_loc);
    if (bits & MASK_MIN)
      fd_enqueue_list_gc(wam, 4, lists_loc);
    if (bits & MASK_DOM)
      fd_enqueue_list_gc(wam, 3, lists_loc);
  }
}
