...
/* d1 x d2 -> {s-t | s in d1 & t in d2 & lb <= s-t <= ub} */
static TAGGED fd_minus(Wam wam,
		       TAGGED d1, TAGGED d2,
		       TAGGED lb, TAGGED ub)
{
  int i, j, count;
...
static int finish_pruning(Wam wam, int why)
{
  ...
  if (why & MASK_VAL) {
	...
	BindHVA(dest_var,value);
 	...
 	h[2] = value;
 	w->global_top = h+3;
 	fd_enqueue_general(wam, MakeStructure(h),0xb);
      }
    });
...
fd_told(Wam wam, int why)
{
...
#if DBG
				/* missing implied bits? */
  if (!(why & MASK_VAL)) {
    TAGGED domain;
    DerefAttribute(domain,X(EVAL_ARITY));
    SP_ASSERT(DomainSize(domain)!=TaggedOne);
  ...
  if (why > 0) {
    ...
    AttrToSuspM(X(EVAL_ARITY),mutable);
    fd_enqueue_all(wam, why,mutable);
  }
}
				 ...
				 SP_term_ref ItemR)
{
  ...
  while (TagIsLST(X(0))) {
	...
	if (IsVar(ent)) {	/* SPRM 12958 */
	  ...
	  status = (RefMutable(statmut)|16); /* STATUS: entailed */
			    ...
			    SP_term_ref GlobalR)
{
  ...
  /*    X(0) = RefTerm(ActionsR); */
  DerefNonvar(X(0));
  switch (fd_prune_and_enqueue(wam,ActionsR,GlobalR,SP_new_term_ref())) {
  case SP_FAILURE:
    fd.failures++;
    SP_fail();
    break;
...
static TAGGED fd_mon_vars(Wam wam,
...
static void link_indexical(Wam wam,
			   struct indexical_info *current)
{				/* link indexical into suspension lists */
  ...
  for (i=0; i<length; i++) {
    ...
    if (!(tmp_link & mask)) {
      t1 = args[tmp_link>>8];
      DerefSwitch(t1,fd_link(wam,t1,fd.linkage_keys[tmp_link & 7],ix););
    }
  }
}
		       ...
		       SP_term_ref ZeroOneAttr)
{
  ...
  count *= 2*arity + 29; /* ix(8) + $mutable(3) + fd_mon_vars(2*arity) + -(3) + iff(5) + link(10) */
  RequireHeap(count,7);	
  ...
  for (type=mintype; type<=maxtype; type++) {
	...
	*q++ = ix;
    ...
    for (ix = *(--q); p<=q; ix = *(--q)) {
      TAGGED vars = fd_mon_vars(wam,ix,X(0)); /* at most 512 words */
      TAGGED item;
	...
	h[7] = X(3);
	w->global_top = h+8;
	fd_link(wam,X(2),fd.functor_val,MakeStructure(h)); /* at most 10 words */
	...
	h[2] = item;
	w->global_top = h+3;
	fd_link(wam,CTagToCar(vars),fd.functor_val,MakeStructure(h)); /* at most 10 words */
		      ...
		      SP_term_ref Global)
{
  ...
  while (TagIsLST(RefTerm(Susp))) {
    ...
    if (IsVar(var) && fun!=fd.functor_none)
      fd_link(wam,var,fun,RefTerm(Global));
  }
  fd_enqueue_append(wam,RefTerm(Global), 0x1);	/* for immediate dequeue */
}
...
prolog_fd_evaluate_indexical(Wam wam, SP_term_ref Global)
{
 ...
 restart:
  ...
  case 2:
    RefTerm(Global) = X(0);
    statem = CTagToArg(X(0),4); /* skip if entailed, can happen if non-idempotent */
  ...
  run_global:
    ...
    FD_UPDATE_MUTABLE(gtemp|IStep(8), statmut);
    fd.resumptions++;
    if (/*!w->heap_warn_soft ||*/ fd.debugging) /* status must be updated even if we return 2 */
      return 2;
    ...
    X(0) = RefTerm(Actions);
    /* X(1) = RefTerm(Global); */
    switch (fd_prune_and_enqueue(wam,Actions,Global,State/*recycled ref*/)) {
    case SP_FAILURE:
      goto fail;
    case SP_ERROR:
 ...
 rerun_ix:
  ...
  DispatchFirst {
	...
	*/
    ...
    CaseX(FD_PRUNE_BOOL):	/* [MC] 4.2.1 */
      {
	TAGGED *pt1;
	SP_integer fun = *code++;
	int shift = 0;

	gtemp = (current->pruned==2) ? atts[0] : atts[current->pruned+1];
	DerefAttribute(gtemp,gtemp);
	pt1 = TagToSTR(gtemp);
	if (pt1[2]!=pt1[3]) {
	  ground = 0;
	} else if (pt1[2]==TaggedZero) {
	  shift += 6;
	} else {
	  shift += 12;
	}
	gtemp = (current->pruned==0) ? atts[2] : atts[current->pruned-1];
	DerefAttribute(gtemp,gtemp);
	pt1 = TagToSTR(gtemp);
	if (pt1[2]!=pt1[3]) {
	  ground = 0;
 ...
 link_ov_loop:
  ...
  FD_UPDATE_MUTABLE(gtemp,X(1));
 ov_loop:
  if (!fd_check_overflow(wam, CTagToArg(X(0),2)))
    goto error;
  goto restart;
 fail:
  fd.failures++;
