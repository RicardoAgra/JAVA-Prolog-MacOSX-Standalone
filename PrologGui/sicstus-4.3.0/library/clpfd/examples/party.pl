...
party(Lab, Consistency) :-
	party_variables(6, Vars),
	party_constraints(6, Vars, Consistency),
...
party_constraints(Times, Vars, Consistency) :-
	sizes(Hosts, Guests),
	(   for(G1,1,Guests),
	    param([Vars,Consistency])
	do  aget(h(G1), Vars, L1),
	    all_different(L1, [consistency(Consistency)])
	),
