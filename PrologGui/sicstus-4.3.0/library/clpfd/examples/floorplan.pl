...
constraints(InOut, ID, Chambers, Shapes, Objects, Classes, Length, Width, Height) :-
	...
	;   Rules = [],
		...
		(   foreach(Name5,Adjacencies4),
		    fromto(DISJ4,Disj0,Disj,0),
		    param(K4,Chamber4,Chambers)
		do  Disj0 = ((X1+L1#=X2 #/\ Y1+W1#>=Y2+K4 #/\ Y2+W2#>=Y1+K4) #\/
		             (X2+L2#=X1 #/\ Y1+W1#>=Y2+K4 #/\ Y2+W2#>=Y1+K4) #\/
		             (Y1+W1#=Y2 #/\ X1+L1#>=X2+K4 #/\ X2+L2#>=X1+K4) #\/
		             (Y2+W2#=Y1 #/\ X1+L1#>=X2+K4 #/\ X2+L2#>=X1+K4) #\/ Disj),
