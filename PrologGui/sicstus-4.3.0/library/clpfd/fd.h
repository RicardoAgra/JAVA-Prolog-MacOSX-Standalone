...
#define FD_NB_QUEUES 6
#define FD_QUEUE_DAEMON 4
#define FD_QUEUE_WAKE 5

struct prop_queue {
  SP_globref items; /* [MC] 4.0.5 */
  int first;
  int last;
  int size;
  int slack;
...
#endif
  struct prop_queue queue[FD_NB_QUEUES]; /* 0=indexical, 1=val, 2=minmax, 3=dom, 4=daemon, 5=wake */
...
extern void fd_enqueue_general(Wam wam, TAGGED item, int where);
extern void fd_enqueue_global(Wam wam, TAGGED item, int where);
extern int  fd_dequeue(Wam wam, TAGGED *item);
extern void fd_enqueue_all(Wam wam, int bits, TAGGED lists_loc);
...
extern void fd_told(Wam wam, int why);
extern void fd_link(Wam wam, TAGGED var, TAGGED key, TAGGED item);

#define fd_enqueue_prepend_ix(wam,item)					\
{									\
  struct prop_queue *q = &cur->queue[0];				\
  if (q->slack==1) {/* must expand */	\
    fd_enqueue_general(wam, item, 0/*prepend*/);			\
  } else {								\
    {								\
    ...
    q->items[--q->first].term[0] = item; /* 4.0.5 [MC] */		\
    q->slack--;                                          		\
  }									\
}
...
{									\
  ...
  if (q->slack==1) {/* must expand */	\
    fd_enqueue_general(wam, item, (ix<<1)/*prepend*/);			\
  } else {								\
    {								\
      TAGGED mutable = CTagToArg(item,3);				\
      FD_UPDATE_MUTABLE(RefMutable(mutable)|IStep(1),mutable); /* STATUS: enqueued */ \
...
#define fd_enqueue_append_ix(wam,item)					\
{									\
  struct prop_queue *q = &cur->queue[0];				\
  if (q->slack==1) {/* must expand */	\
    fd_enqueue_general(wam, item, 0x1/*append*/);			\
  } else {								\
    {								\
      TAGGED mutable = CTagToArg(item,3); 				\
      FD_UPDATE_MUTABLE(RefMutable(mutable)|IStep(1),mutable); /* STATUS: enqueued */ \
    ...
    q->items[q->last++].term[0] = item;	/* 4.0.5 [MC] */		\
...
{									\
  struct prop_queue *q = &cur->queue[ix];				\
  if (q->slack==1) {/* must expand */	\
    fd_enqueue_general(wam, item, (ix<<1)+0x1/*append*/);			\
  } else {								\
    {								\
      TAGGED mutable = CTagToArg(item,3); 				\
      FD_UPDATE_MUTABLE(RefMutable(mutable)|IStep(1),mutable); /* STATUS: enqueued */ \
...
#define fd_enqueue_daemon(wam,item)					\
{									\
  struct prop_queue *q = &cur->queue[4];				\
  if (q->slack==1) {/* must expand */	\
    fd_enqueue_general(wam, item, 9/*append*/);				\
  } else {								\
    q->items[q->last++].term[0] = item; /* 4.0.5 [MC] */		\
    if (q->last==q->size) q->last = 0;					\
    q->slack--;                                          		\
