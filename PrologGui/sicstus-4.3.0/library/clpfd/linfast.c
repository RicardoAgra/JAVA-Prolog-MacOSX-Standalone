	   ...
	   TAGGED global)
{
  ...
  TAGGED state_stamp = GetSmall(CTagToArg(tstate,ar))-1; /* was inc'ed above */
  int enable;
  TAGGED tmin, tmax;
  ...
  if (VS(varno)>=pdata->ntargets)
    return;			/* if varno not among targets then self-invocation */
		  ...
		  SP_term_ref Actions)
{
  ...
  SP_integer state_stamp;
  int nvars, i, j, k, ent = -1, change, ntargets0;
  SP_integer total_size;
  char *ptr;
...
/*    X(0) = RefTerm(State0); */
  ...
  do {
    ...
    for (i=0; i<pdata->ntargets; i++) {
      ...
      if (imin<imax && GCD(elt)>pdata->gcdall /* && ABSCOEFF(elt)!=1 */) {
	/* Ensure that:
