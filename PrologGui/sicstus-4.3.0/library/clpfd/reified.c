	      ...
	      TAGGED global)
{
  ...
  (void)attr_ref;
  dvar_init(dvx+0, refbase,   refbase+1);
  dvar_init(dvx+1, refbase+2, refbase+3);
  dvar_init(dvx+2, refbase+4, refbase+5);
  ...
  return;
 push:
  fd_enqueue_global(wam, global, 0x2/* VAL, prepend*/);   
}
...
/*    X(0) = RefTerm(State0); */
  ...
  if (ent==0 && post) {
    if (dvar_is_integer(dvb)) {
      ...
      dvar_attach_daemon(wam, dvx, pdata, X(1), fd.functor_dom);
      dvar_attach_daemon(wam, dvy, pdata, X(1), fd.functor_dom);
