...
/***
  ...
  Algorithms:
  Regin '94, Costa '94, Caseau & Laburthe '97, Mehlhorn & Thiel '00,
  and more for circuit.
  ***/
		    ...
		    TAGGED global)
{
  ...
  return;
 post:
    fd_enqueue_global(wam, global, pdata->enqueue_code);
}
				     ...
				     VERTEX vertex)
{
  ...
  SP_BOOL seen_major = TRUE;	/* seen edge to latest major subtree? */
  VERTEX child;
  VERTEX *vstack = pdata->dist_stack;
  SP_integer *lstack = (SP_integer *)vstack;
...
start:
  ...
  while (!dviter_empty(&it)) {
    ...
    child = pdata->tail[neighbor_of(wam, pdata,tv,&valtarget, vertex)-nvars /*LABEL(neighs[j])-1*/];
    m = (int)VISITED(child);
    if (m==0) {
      vstack[top] = vertex;
      tstack[top+1] = tv;
  ...
  if (min==vertid)
    return (min==1 && id==pdata->nvarkernels);
  m = min;
  top -= 3;
  vertex = vstack[top];
  tv = tstack[top+1];
  ...
  if (VISITED(vertex)==1) {	/* just visited a major subtree */
    if (!seen_major && tv!=dvar_min_t(DVAR(vertex))) /* which must have an edge into the subtree */
      return FALSE;		/* to its left */
    majormin = vertid;
    majormax = id;
    seen_major = FALSE;
  }
  vertid = (int)VISITED(vertex);
  dviter_init(&it, DVAR(vertex));
  dviter_skip_t(&it, tv);
  goto cont;
}
...
/* the targets need to be ordered (see ORDER comments) */
static void contract_vals (struct all_dist_data *pdata)
{
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    nvals = nvars*nvars;
    if (AreSmall(fd_min(all),fd_max(all)) && fd_size(all)<nvals) {
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!pdata->cost ? !ass_filter(wam, pdata) :
      !cost_filter(wam, pdata,GetSmall(CTagToArg(X(0),6)),flags))
    goto ret;
			   ...
			   int flags)
{
  ...
  dvar_init(pdata->rc.costvar, RefAttr(nvars<<1), RefVar(nvars<<1));
  ...
  if (pdata->incremental)
    ...
    for (i=0; i<nvaltargets; i++) {
      ...
      pdata->rc.var_index[vertex] = (int)n;
      pdata->rc.index_var[n++] = (int)vertex;
      MATE(vertex) = vertex;	/* for contract_vals() */
    }
    n >>= 1;
...
/*    X(0) = RefTerm(State0); */
  ...
  dv = pdata->rc.costvar;
  dvar_init(dv, RefAttr(nvars<<1), RefVar(nvars<<1));
