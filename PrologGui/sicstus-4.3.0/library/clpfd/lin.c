...
div_dud(Wam wam, Dword x,SP_UWORD y,Dword z)
{
  ...
  if (y==1) {
    ...
    z->msw.asuword = 0;
    {
      ...
      COMPILE_TIME_ASSERT( ((~(SP_UWORD)0)<<(WORDSIZE-1)) == ((SP_UWORD)(((SP_WORD)-1)<<(WORDSIZE-1))) );
	      ...
	      TAGGED global)
{
  ...
  if (pdata->nonground==0) {
    ...
    case 1:
      elt = pdata->heap[0];
      if (cmp_dltd(pdata->bigf,INTERVAL(elt)) || cmp_dlez(pdata->bige))
	push = 1;
      break;
    case 2:
      elt = pdata->heap[0];
      if (cmp_dltd(pdata->bige,INTERVAL(elt)) | cmp_dlez(pdata->bigf))
	push = 1;
      break;
    case 3:
      elt = pdata->heap[0];
 ...
 loop:
  ...
  case 1:			/* #=< */
    if (pdata->nonground>0) {
      ...
      if (cmp_dltd(pdata->bigf,INTERVAL(elt))) {
	switch (scalar_product_le(wam, pdata,elt)) {
	case 1:
	  goto loop;
	case 2:
	  goto out;
  ...
  case 2:			/* #>= */
	...
	switch (scalar_product_ge(wam, pdata,elt)) {
	case 1:
	  goto loop;
	case 2:
	  goto out;
