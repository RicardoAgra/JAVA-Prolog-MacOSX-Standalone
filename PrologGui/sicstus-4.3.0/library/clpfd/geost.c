...
#if 0
static void
expand_forbidden(struct pdata *pdata,
		 int f,
		 int pruningmin,
		 SP_integer *jumpv,
		 int &cookie)
{
  if (pruningmin) {
    SP_integer v = FR_END(*cookie,dim);
    
    FR_END(f,dim) = v < pdata->next[dim]-1 ? v : pdata->next[dim]-1;
  } else {
    SP_integer v = FR_ORIGIN(*cookie,dim);
    
    FR_ORIGIN(f,dim) = v > pdata->next[dim]+1 ? v : pdata->next[dim]+1;
  }
}
#endif
...
#if 0
static SP_BOOL
pl_convex_hull_feasible(Wam wam,
			      struct geost_data *pdata)
{
  int kdims = pdata->kdims;
  int s = pdata->pl.sbox;
  ...
  for (i=0; i<pdata->ntargets; i++) {
    ...
    if (!object_is_ground(pdata,io)) {
      bb_add_object(wam, pdata,/*bounding box*/2,io);
      nng++;
    }
  }
  if (nng < 2)
    return TRUE;
  length = FR_END(/*bounding box*/2,0) - FR_ORIGIN(/*bounding box*/2,0) + 1;
  height = FR_END(/*bounding box*/2,1) - FR_ORIGIN(/*bounding box*/2,1) + 1;
  x = length > height ? length : height;
  y = length < height ? length : height;
  for (i=0; i<pdata->nobjects; i++) {
    int io = TARGET(i);
    if (object_is_ground(pdata,io)) {
      ...
      SP_integer sid = dvar_min_l(dvs);
      int s1 = get_first_sbox(sid);
      SP_integer xmin = dvar_min_l(dv+0);
      SP_integer xmax = xmin + S_SIZE(s1,0)-1;
      SP_integer ymin = dvar_min_l(dv+1);
      SP_integer ymax = ymin + S_SIZE(s1,1)-1;

      if (xmax>=FR_ORIGIN(2,0) && FR_END(2,0)>=xmin &&
	  ymax>=FR_ORIGIN(2,1) && FR_END(2,1)>=ymin) {
	if (!overlap) {
	  overlap = TRUE;
	  FR_ORIGIN(3,0) = (xmin > FR_ORIGIN(2,0) ? xmin : FR_ORIGIN(2,0));
	  FR_END(3,0) = (xmax < FR_END(2,0) ? xmax : FR_END(2,0));
	  FR_ORIGIN(3,1) = (ymin > FR_ORIGIN(2,1) ? ymin : FR_ORIGIN(2,1));
	  FR_END(3,1) = (ymax < FR_END(2,1) ? ymax : FR_END(2,1));
	} else if (xmin-1==FR_END(3,0) && ymin==FR_ORIGIN(3,1) && ymax==FR_END(3,1)) { /* extend bb 3 right */
	  FR_END(3,0) = (xmax < FR_END(2,0) ? xmax : FR_END(2,0));
	} else if (xmax+1==FR_ORIGIN(3,0) && ymin==FR_ORIGIN(3,1) && ymax==FR_END(3,1)) { /* extend bb 3 left */
	  FR_ORIGIN(3,0) = (xmin > FR_ORIGIN(2,0) ? xmin : FR_ORIGIN(2,0));
	} else if (ymin-1==FR_END(3,1) && xmin==FR_ORIGIN(3,0) && xmax==FR_END(3,0)) { /* extend bb 3 right */
	  FR_END(3,1) = (ymax < FR_END(2,1) ? ymax : FR_END(2,1));
	} else if (ymax+1==FR_ORIGIN(3,1) && xmin==FR_ORIGIN(3,0) && xmax==FR_END(3,0)) { /* extend bb 3 left */
	  FR_ORIGIN(3,1) = (ymin > FR_ORIGIN(2,1) ? ymin : FR_ORIGIN(2,1));
	} else {
	  return TRUE;
	}
  ...
  if (overlap) {
    SP_integer gap_left  = FR_ORIGIN(3,0) - FR_ORIGIN(2,0);
    SP_integer gap_right = FR_END(2,0) - FR_END(3,0);
    SP_integer gap_down  = FR_ORIGIN(3,1) - FR_ORIGIN(2,1);
    SP_integer gap_up    = FR_END(2,1) - FR_END(3,1);
    SP_integer slack = x*y - nng*a*b;
    SP_integer ov_length = FR_END(3,0) - FR_ORIGIN(3,0) + 1;
    SP_integer ov_height = FR_END(3,1) - FR_ORIGIN(3,1) + 1;
    SP_integer loss = ov_length*ov_height;

    while (!islin[gap_left]) {
      gap_left--;
      loss += ov_height;
    }
    while (!islin[gap_right]) {
      gap_right--;
      loss += ov_height;
    }
    while (!islin[gap_down]) {
      gap_down--;
      loss += ov_length;
    }
    while (!islin[gap_up]) {
    ...
    if (!pl_is_feasible(wam, nng,x,y,a,b,islin))
      return FALSE;
  }
  return TRUE;
}
			   ...
			   SP_integer sslack)
{
  ...
  switch (msg) {
  case 1:			/* gethash */
    node = incore_gethash(pdata->dp_hash[curdim],hash_tmp[0])->value.any;
    while (node) {
    ...
    break;
  case 2:			/* puthash */
    keyval = dyn_puthash(&pdata->dp_hash[curdim],hash_tmp[0]);
#if DP_HASH_BOUNDED
		 ...
		 int dim)
{
  ...
  for (i=0, ev=0; i<pdata->nobjects; i++) { /* for all objects intersecting BB 1 */
	...
	if (lst < ect) {
	  event[ev].key = (lst<<1)+1;
	  event[ev].object = object;
	  event[ev].sbox = s;
	  event[ev+1].key = (ect<<1);
	    ...
	    int init_bounds)
{
  ...
  SP_globref ref = O_SID_ATTR(object);
  int j;
  
  dvar_init(dv, ref, ref+1);
  dv = O_ORIG_DVAR(object,0);
  for (j=0; j<kdims; j++) {
    ref = O_ORIG_ATTR(object,j);
	     ...
	     TAGGED global)
{
  ...
  if (!(RefMutable(tmp) & IStep(1))) {	/* STATUS: not enqueued */
    ...
    pdata->stamp++;
    fd_enqueue_global(wam, global, 0x5);
  }
}
...
/*    X(0) = RefTerm(State0); */
  ...
  for (i=0; i<pdata->ntargets; i++)
   ...
   {
     SP_globref ref = FIX_ATTR;
     dvar_init(pdata->fix_dvar, ref, ref+1);
   }
   if (pdata->opt & OPT_VOLUME) {
     SP_globref ref = VOLUME_ATTR;
     dvar_init(pdata->volume_dvar, ref, ref+1);
     dvar_fix_max_l(pdata->volume_dvar,HighInt-1); /* no 'sup' please */
   }
   for (j=0; j<kdims; j++) {
     SP_globref ref = LOWER_ATTR+2*j;
     dvar_init(pdata->lower_dvar+j, ref, ref+1);
     dvar_fix_interval_l(pdata->lower_dvar+j,-HighInt,HighInt-1); /* no 'inf'/'sup' please */
   }
   for (j=0; j<kdims; j++) {
     SP_globref ref = UPPER_ATTR+2*j;
     dvar_init(pdata->upper_dvar+j, ref, ref+1);
