...
#if SICSTUS_RELEASE_BUILD && !FORCE_BUILD && !FORCE_BUILD_TK_C && SICSTUS_VERSION > 40300 /* [PM] 4.3 postpone */
#error "[PM] 4.2 Make it posssible to specify an alternative Tcl/Tk framework path on Mac OS X (SPRM 11962)"
     ...
     int fd;
{
  ...
  if (Tk_GetNumMainWindows() > 0)
    {
      (void) Tcl_DoOneEvent(0);
      FD_ZERO(&dread);
#error "[PM] 4.1.3 something like this would be needed to prevent buffer overflow in FD_SET"
      if (! (0 <= fd && fd < FD_SETSIZE) ) /* SPRM 11822 */
        {
          return -1;
        }
