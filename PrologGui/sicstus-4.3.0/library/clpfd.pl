...
fdlists_suspensions(Lists) -->
	...
	{arg(6, Lists, L6)},
	{arg(7, Lists, L7)},
	(foreach(X3,L3) do suspension(X3)),
	(foreach(X4,L4) do suspension(X4)),
	(foreach(X5,L5) do suspension(X5)),
	(foreach(X6,L6) do suspension(X6)),
	(foreach(X7,L7) do suspension(X7)).

suspension(_-Item) --> !,
	suspension(Item).
suspension(iff(Ix,B,K,A)) -->
	{var(A)},
	{Ix = ix(Ptr,FDPred,_,Ent,_,_,_)},
	{var(Ent)}, !,
...
suspension(ix(Ptr,FDPred,_,Ent,ZeroOne,_,_)) -->
	{var(Ent)}, !,
	...
	suspension_item(Type, ZeroOne, Module:Constraint).
suspension(global(_,_,_,Ent,clpfd:FDPred)) -->
	{var(Ent)},
...
suspension_item(_, B, C) --> {B==0}, !, [#\ C].
suspension_item(_, B, C) --> {B==1}, !, [C].
suspension_item(0, _, C) --> [#\ C].
suspension_item(1, _, C) --> [C].
suspension_item(2, B, C) --> [C #<=> B].
suspension_item(3, B, C) --> [C #<=> B].
...
fdpred_exported('oneof(x,y)=z IND'(X,Y,Z), oneof(X,Y,Z)).
...
fdpred_exported(bool(0,P,Q,B), P   #/\  Q #<=> B).
fdpred_exported(bool(1,P,Q,B), P   #/\  #\Q #<=> B).
fdpred_exported(bool(2,P,Q,B), #\P #\/  #\Q #<=> B).
fdpred_exported(bool(3,P,Q,B), P   #\/  Q #<=> B).
fdpred_exported(bool(4,P,Q,B), P   #\/  #\Q #<=> B).
fdpred_exported(bool(5,P,Q,B), #\P #/\  #\Q #<=> B).
fdpred_exported(bool(6,P,Q,B), P   #\   Q #<=> B).
fdpred_exported(bool(7,P,Q,B), P   #<=> Q #<=> B).
...
foreign(prolog_fd_assert_no_leak, '$fd_assert_no_leak').
...
foreign(prolog_fd_dc_linear, '$fd_dc_linear'(+term,-term,-term)).
