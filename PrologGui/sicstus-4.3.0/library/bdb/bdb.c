...
#if 0 /* [PM] 4.3 no longer use dbaux */
#include "dbaux.h"
#endif
...
/* [PM] 4.1.2 Postpone since we do not use any of the APIs that
        ...
        idea, eventually. */
#if !FORCE_BUILD && SICSTUS_RELEASE_BUILD && SICSTUS_VERSION > 40300 /* [PM] 4.3 postpone */
/*
...
void SPCDECL set_fastrw_compatibility(SP_integer backward_compatible)
{
#if BDB_DEBUG > 1
    fprintf(stderr, "%s()@%s:%d ENTER\n", __FUNCTION__, __FILE__, (int)__LINE__);
#endif
...
void SPCDECL get_fastrw_compatibility(SP_integer *backward_compatible)
{
#if BDB_DEBUG > 1
    fprintf(stderr, "%s()@%s:%d ENTER\n", __FUNCTION__, __FILE__, (int)__LINE__);
#endif
